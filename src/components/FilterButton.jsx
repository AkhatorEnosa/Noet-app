...
const FilterButton = ({ sortValue, setSortValue, options, showSortOptions, setShowOptions }) => {
  ...
  return (
    <div className="relative h-fit">
        ...
        <ul className={`${showSortOptions ? "scale-100" : "scale-0"} lg:w-52 top-16 left-0 divide-y bg-white border-[1px] border-gray-300 rounded-md shadow-md absolute text-xs transition-all duration-150 ease-in-out overflow-hidden z-[62]`}>
            {options.map((option) => (
                <li key={option} className={`capitalize p-2 pr-14 md:pr-24 lg:pr-0 hover:bg-blue-[#114f60]/10 ${sortValue == option && 'font-extrabold bg-[#114f60]/10'} cursor-pointer transition-all duration-200`} onClick={() => setShowOptions(false) & setSortValue(option)}>{option}</li>
            ))}
        </ul>
    </div>
  )
